...
This is a **Next.js 14+ App Router** project deployed on **Vercel** at `https://www.towingno1.com`.
...
### Open Graph
Update `openGraph.title`, `openGraph.description`, and `openGraph.url` to match the page title/description and page canonical URL. The OG image at `https://www.towingno1.com/preview.jpg` is used sitewide — replace with a page-specific image if available.
...
The sitemap is auto-generated at `https://www.towingno1.com/sitemap.xml` by `app/sitemap.ts`.
...
1. Check it renders correctly: `https://www.towingno1.com/sitemap.xml`
2. Submit to Google Search Console → Sitemaps → `https://www.towingno1.com/sitemap.xml`
...
- Disallow `api/` paths
- Allow Next.js static assets (including `/_next/`) so search engines can render pages correctly
- Points to canonical sitemap URL: `https://www.towingno1.com/sitemap.xml`
...
The `metadataBase` in `layout.tsx` is set to `https://www.towingno1.com`. All relative canonical URLs resolve against this base.
...
2. Add property: `https://www.towingno1.com`
3. Verify via HTML tag, DNS record, or Google Analytics
4. Go to **Sitemaps** → Enter `https://www.towingno1.com/sitemap.xml` → Submit
